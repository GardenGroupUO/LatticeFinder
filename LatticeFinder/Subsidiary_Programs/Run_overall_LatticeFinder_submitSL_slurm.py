#!/usr/bin/env python3
'''
Geoffrey Weal, Run_submitSL_slurm.py, 10/02/2021
...
def get_number_to_trials_that_will_be_submitted_by_submitSL():
    return 1

command = "squeue -r -u $USER"
def check_max_jobs_in_queue_after_next_submission():
    while True:
        text = myrun(command)
    ...
    number_of_trials_to_be_submitted = get_number_to_trials_that_will_be_submitted_by_submitSL()
...
for file_to_submit in files_to_submit:
    ...
    while True:
        ...
        if reached_max_jobs:
            ...
            break
    # submit the jobs
    print("Submitting " + str(file_to_submit) + " to slurm.")
    ...
    while True:
        if error_counter == number_of_consecutive_error_before_exitting:
            ...
            proc = subprocess.Popen(submitting_command+' '+file_to_submit, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            if not proc.wait() == 0:
                error_counter += 1
                if error_counter == number_of_consecutive_error_before_exitting:
                    print('----------------------------------------------')
